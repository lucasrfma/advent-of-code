...
public class Day6(StreamReader sr, ILogger logger)
{
    ...
    internal long NumberOfWays(TimeAndRecord tr)
    {
        ...
        double max = (-b - sqrtD) / 2 * a;
        double min = (-b + sqrtD) / 2 * a;
        long lmax = (long)max;
        long lmin = (long)min;
        if (lmax == Math.Round(max,5)) lmax--;

        logger.LogInformation($"a: {a} b: {b} c: {c} sqrtD: {sqrtD} min: {min} max: {max} lmin: {lmin} lmax: {lmax}");
        return lmax - lmin;
    }
